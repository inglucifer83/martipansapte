    ...
    <div style="padding: 2vw 2vw;">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <b style="font-size: 2rem;">Variants</b>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="variants-table" class="table table-striped data-table">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Product Id</th>
                        ...
                        <tbody>
                            @foreach ($variants as $variant)
                                <tr>
                                    <td style="vertical-align: middle;">{{ $variant->id }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->product_id }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->sku }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->price }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->compare_at_price }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->inventory_quantity }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->attributes }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->weight }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->image_id }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->created_at }}</td>
                                    <td style="vertical-align: middle;">{{ $variant->updated_at }}</td>
                                        ...
                                        <div
                                            ...
                                            @hasanyrole('SuperAdmin|Manager|Editor')
                                                <button type="button" data-bs-toggle="modal"
                                                    data-bs-target="#variant-modal" data-id="{{ $variant->id }}"
                                            ...
                                            @endhasanyrole
                                            <a role="button"
                                                href="{{ route('admin.variants.variant', $variant->id) }}"
                                            ...
                                            @hasanyrole('SuperAdmin|Manager')
                                                <form action="{{ route('admin.variants.delete', $variant->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="btn btn-sm btn-outline-danger"
                                                        data-modal="form" data-type="danger" data-title="Delete Variant"
                                                        data-body="Are you sure you want to delete the user with id {{ $variant->id }}?"
                                            ...
                                            @if ($variant->trashed())
                                                @hasanyrole('SuperAdmin|Manager')
                                                    <form action="{{ route('admin.variants.restore', $variant->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <button type="button" class="btn btn-sm btn-outline-success"
                                                            data-modal="form" data-type="success"
                                                            data-title="Restore Variant"
                                                            data-body="Are you sure you want to delete the user with id {{ $variant->id }}?"
                                                ...
                                                @endhasanyrole
                                                @role('SuperAdmin')
                                                    <form action="{{ route('admin.variants.erease', $variant->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-sm btn-outline-danger"
                                                            data-modal="form" data-type="danger" data-title="Erease Variant"
                                                            data-body="Are you sure you want to delete the user with id {{ $variant->id }}?"
