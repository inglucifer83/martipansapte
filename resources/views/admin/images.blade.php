    ...
    <div style="padding: 2vw 2vw;">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <b style="font-size: 2rem;">Images</b>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="images-table" class="table table-striped data-table">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Url</th>
                        ...
                        <tbody>
                            @foreach ($images as $image)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle;">{{ $image->alt_text }}</td>
                                    <td style="vertical-align: middle;">{{ $image->caption }}</td>
                                    <td style="vertical-align: middle;">{{ $image->position }}</td>
                                    <td style="vertical-align: middle;">{{ $image->is_primary }}</td>
                                    <td style="vertical-align: middle;">{{ $image->product_id }}</td>
                                    <td style="vertical-align: middle;">{{ $image->created_at }}</td>
                                    <td style="vertical-align: middle;">{{ $image->updated_at }}</td>
                                    <td class="admin-action-buttons-column" style="vertical-align: middle;">
                                        <div
                                            ...
                                            @hasanyrole('SuperAdmin|Manager|Editor')
                                                <button type="button" data-bs-toggle="modal" data-bs-target="#image-modal"
                                                    data-id="{{ $image->id }}" class="btn btn-sm btn-outline-warning"
                                            ...
                                            @endhasanyrole
                                            <a role="button" href="{{ route('admin.images.image', $image->id) }}"
                                            ...
                                            @hasanyrole('SuperAdmin|Manager')
                                                <form action="{{ route('admin.images.delete', $image->id) }}"
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-outline-danger"
                                                        data-modal="form" data-type="danger" data-title="Delete Image"
                                                        data-body="Are you sure you want to delete the user with id {{ $image->id }}?"
                                            ...
                                            @if ($image->trashed())
                                                @hasanyrole('SuperAdmin|Manager')
                                                    <form action="{{ route('admin.images.restore', $image->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <button type="button" class="btn btn-sm btn-outline-success"
                                                            data-modal="form" data-type="success" data-title="Restore Image"
                                                            data-body="Are you sure you want to delete the user with id {{ $image->id }}?"
                                                ...
                                                @endhasanyrole
                                                @role('SuperAdmin')
                                                    <form action="{{ route('admin.images.erease', $image->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-sm btn-outline-danger"
                                                            data-modal="form" data-type="danger" data-title="Erease Image"
                                                            data-body="Are you sure you want to delete the user with id {{ $image->id }}?"
