    ...
    <div style="padding: 2vw 2vw;">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <b style="font-size: 2rem;">Categories</b>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="categories-table" class="table table-striped data-table">
                        <thead>
                            <tr>
                                <th>Id</th>
                                <th>Name</th>
                            ...
                            @foreach ($categories as $category)
                                <tr>
                                    ...
                                    <td style="vertical-align: middle;"><img
                                            src="{{ Str::startsWith($category->image, 'http') ? $category->image : asset('storage/' . $category->image) }}"
                                            width="60" height="60" style="border-radius: 0%;" /></td>
                                    <td style="vertical-align: middle;">{{ $category->parent_id }}</td>
                                    <td style="vertical-align: middle;">{{ $category->sort_order }}</td>
                                    <td style="vertical-align: middle;">{{ $category->seo_title }}</td>
                                    <td style="vertical-align: middle;">{{ $category->seo_description }}</td>
                                    <td style="vertical-align: middle;">{{ $category->is_active }}</td>
                                    <td style="vertical-align: middle;">{{ $category->created_at }}</td>
                                    <td style="vertical-align: middle;">{{ $category->updated_at }}</td>
                                    <td class="admin-action-buttons-column" style="vertical-align: middle;">
                                        <div
                                            ...
                                            @endhasanyrole
                                            <a role="button"
                                                href="{{ route('admin.categories.category', $category->id) }}"
                                            ...
                                            @hasanyrole('SuperAdmin|Manager')
                                                <form action="{{ route('admin.categories.delete', $category->id) }}"
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-outline-danger"
                                                        data-modal="form" data-type="danger" data-title="Delete Category"
                                                        data-body="Are you sure you want to delete the user with id {{ $category->id }}?"
                                            ...
                                            @if ($category->trashed())
                                                ...
                                                @endhasanyrole
                                                @role('SuperAdmin')
                                                    <form action="{{ route('admin.categories.erease', $category->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-sm btn-outline-danger"
                                                            data-modal="form" data-type="danger"
                                                            data-title="Erease Category"
                                                            data-body="Are you sure you want to delete the user with id {{ $category->id }}?"
