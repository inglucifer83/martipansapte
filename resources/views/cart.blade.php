<x-html lang="{{ app()->getLocale() }}">
    <x-slot:head>
        ...
        <meta name="keywords" content="cart, checkout, basket">
        <meta property="og:title" content="Shopping Cart - MyShop">
        <meta name="description"
            content="View your cart items, update quantities and proceed to checkout with a clear summary of totals.">
    ...
    <div style="width: 100vw; min-height: 100vh" class="d-flex flex-column ">
            ...
            <main class="d-flex flex-column justify-content-start align-items-stretch container">
                ...
                <div style="flex-wrap: wrap" class="d-flex justify-content-center ">
                    @foreach ($cartItems as $item)
                        <div style="padding-top: 1rem; padding-bottom: 1rem; border-bottom: 1px solid rgba(0,0,0,0.05)"
                            class="d-flex justify-content-between align-items-center ">
                            <div class="d-flex justify-content-start align-items-center ">
                                <img style="min-width: 4rem; max-width: 4rem; object-fit: cover; margin-right: 1rem"
                                    src="{{ $item->product->featured_image }}" alt="{{ $item->product->name }}">
                            ...
                            <div class="d-flex justify-content-end align-items-center ">
                                <form id="form-1" action="{{ url('/') }}" method="POST"
                                    class="d-flex justify-content-start align-items-center " method="post">
                                    @csrf
                                    @method('post')
                                ...
                                <form id="form-2" action="{{ url('/') }}" method="POST"
                                    class="d-flex flex-column justify-content-start align-items-stretch "
                                    method="post">
                                    @csrf
                                    @method('post')
                ...
                <div style="padding-top: 1rem" class="d-flex justify-content-between align-items-center ">
                    <div class="d-flex flex-column justify-content-start align-items-stretch ">
                        <h3>
                            {{ __('cart.SUBTOTAL') }}
                        </h3>
                    ...
                    <a id="a-9" href="{{ route('checkout.show') }}" class=" btn btn-ht-primary">
        ...
        <footer style="padding: 0.5vw 1vw; background-color: var(--primary)"
            ...
            <nav class="d-flex justify-content-center align-items-center ">
                <a id="a-10" href="{{ route('about') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.ABOUT') }}
                </a>
                <a id="a-11" href="{{ route('contact') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.CONTACT') }}
                </a>
                <a id="a-12" href="{{ route('products.index') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.SHOP') }}
                </a>
                <a id="a-13" href="{{ route('orders.index') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.ORDERS') }}
                </a>
            </nav>
            <form id="form-3" action="{{ url('/') }}" method="GET">
        ...
        <footer style="padding: 0.5vw 1vw; background-color: var(--background)"
            ...
            <nav style="flex: 1" class="d-flex justify-content-center align-items-center ">
                <a id="a-14" href="{{ route('privacy') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--primary)">
                    {{ __('labels.PRIVACY_POLICY') }}
                </a>
                <a id="a-15" href="{{ route('terms') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--primary)">
                    {{ __('labels.TERMS_OF_SERVICE') }}
                </a>
                <a id="a-16" href="{{ route('cookies') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--primary)">
                    {{ __('labels.COOKIE_POLICY') }}
                </a>
            </nav>
            <div style="flex: 0.5" class="d-flex justify-content-end align-items-center ">
                <a id="a-17" href="{{ url('/lang/it') }}" style="text-decoration: none; margin: 0 0.5vw">
                    {{ __('labels.🇮🇹') }}
                </a>
                <a id="a-18" href="{{ url('/lang/en') }}" style="text-decoration: none; margin: 0 0.5vw">
                    {{ __('labels.🇬🇧') }}
                </a>
            </div>
        </footer>
