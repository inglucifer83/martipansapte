<x-html lang="{{ app()->getLocale() }}">
    <x-slot:head>
        ...
        <meta name="keywords" content="products, shop, catalog">
        <meta property="og:title" content="Shop - MyShop">
        <meta name="description"
            content="Browse MyShop product listings with filters, sorting and pagination to help customers discover the right products.">
    ...
    <div style="width: 100vw; min-height: 100vh" class="d-flex flex-column ">
        ...
        <div style="flex: 1; min-height: 100vh; background-color: var(--background); padding: 2vw 4vw"
            ...
            <main class="d-flex flex-column justify-content-start align-items-stretch ">
                <section class="d-flex flex-column justify-content-start align-items-stretch container">
                    <h1>
                        {{ __('products.PRODUCTS') }}
                    </h1>
                    <div class="d-flex justify-content-between align-items-start ">
                        <aside style="min-width: 14rem; margin-right: 1rem"
                            class="d-flex flex-column justify-content-start align-items-stretch ">
                            <h2>
                                {{ __('products.FILTERS') }}
                            </h2>
                            <div class="d-flex flex-column justify-content-start align-items-stretch ">
                                ...
                                <div style="flex-wrap: wrap" class="d-flex justify-content-center ">
                                    @foreach ($categories as $category)
                                        <a id="a-9" href="{{ route('products.index') }}" class=" btn">
                                            {{ $category->name }}
                                        </a>
                                    @endforeach
                                </div>
                                ...
                                <div style="flex-wrap: wrap" class="d-flex justify-content-center ">
                                    @foreach ($tags as $tag)
                                        <a id="a-10" href="{{ route('products.index') }}" class=" btn">
                                            {{ $tag->name }}
                                        </a>
                                    @endforeach
                                </div>
                        ...
                        <section style="flex: 1" class="d-flex flex-column justify-content-start align-items-stretch ">
                            <div style="flex-wrap: wrap" class="d-flex justify-content-center ">
                                @foreach ($products as $product)
                                    <article
                                        style="padding-top: 1rem; padding-bottom: 1rem; border-bottom: 1px solid rgba(0,0,0,0.05)"
                                        class="d-flex flex-column justify-content-start align-items-stretch ">
                                        <a id="a-11" href="{{ route('products.show', [$product->id]) }}"
                                            class="d-flex justify-content-start align-items-center ">
                                            <img style="min-width: 6rem; max-width: 6rem; object-fit: cover; margin-right: 1rem"
                            ...
                            <nav aria-label="Pagination">
                                <a id="a-12" href="{{ route('products.index') }}" class=" btn">
                                    {{ __('products.PREVIOUS') }}
                                </a>
                                <a id="a-13" href="{{ route('products.index') }}" class=" btn">
                                    {{ __('products.NEXT') }}
                                </a>
                            </nav>
                        </section>
        ...
        <footer style="padding: 0.5vw 1vw; background-color: var(--primary)"
            ...
            <nav class="d-flex justify-content-center align-items-center ">
                <a id="a-14" href="{{ route('about') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.ABOUT') }}
                </a>
                <a id="a-15" href="{{ route('contact') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.CONTACT') }}
                </a>
                <a id="a-16" href="{{ route('products.index') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.SHOP') }}
                </a>
                <a id="a-17" href="{{ route('orders.index') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--background)">
                    {{ __('labels.ORDERS') }}
                </a>
            </nav>
        ...
        <footer style="padding: 0.5vw 1vw; background-color: var(--background)"
            ...
            <nav style="flex: 1" class="d-flex justify-content-center align-items-center ">
                <a id="a-18" href="{{ route('privacy') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--primary)">
                    {{ __('labels.PRIVACY_POLICY') }}
                </a>
                <a id="a-19" href="{{ route('terms') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--primary)">
                    {{ __('labels.TERMS_OF_SERVICE') }}
                </a>
                <a id="a-20" href="{{ route('cookies') }}"
                    style="margin-left: 1vw; margin-right: 1vw; text-decoration: none; color: var(--primary)">
                    {{ __('labels.COOKIE_POLICY') }}
                </a>
            </nav>
            <div style="flex: 0.5" class="d-flex justify-content-end align-items-center ">
                <a id="a-21" href="{{ url('/lang/it') }}" style="text-decoration: none; margin: 0 0.5vw">
                    {{ __('labels.🇮🇹') }}
                </a>
                <a id="a-22" href="{{ url('/lang/en') }}" style="text-decoration: none; margin: 0 0.5vw">
                    {{ __('labels.🇬🇧') }}
                </a>
            </div>
        </footer>
